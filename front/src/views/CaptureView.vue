            ...
            <div class="preview-content">
              <template v-if="onionSkinDisplay">
                <img
                  v-if="
                    getActiveShot &&
                    getActiveShot.images[activeFrame - onionSkinValue] &&
                    activeDevice &&
                    IsFrameLiveView &&
                    onionSkinDisplay > 0
                  "
                  ...
                  :src="
                    ImageCacheService.getImage(
                      getActiveShot.images[activeFrame - onionSkinValue].id
                    )
                  "
                />
                <template v-for="ghostIndex in onionSkinAsArray">
                  <img
                    ...
                    v-if="
                      getActiveShot &&
                      getActiveShot.images[activeFrame - ghostIndex] &&
                      activeDevice &&
                      IsFrameLiveView
                    "
                    alt="ghostImg"
                    ...
                    :src="
                      ImageCacheService.getImage(
                        getActiveShot.images[activeFrame - ghostIndex].id
                      )
                    "
                  />
                </template>
              ...
              <video
                ...
                :style="{
                  ...
                  opacity: onionSkinDisplay ? 0.4 : 1,
                }"
                autoplay
                muted
                playsinline
              ...
              <img
                ...
                :src="
                  ImageCacheService.getImage(
                    getActiveShot.images[draggingPreviewIndex].id
                  )
                "
                :class="{ hidden: IsFrameLiveView }"
              />
            ...
            <div class="play-controls">
              ...
              <template v-if="!isMultiSelect">
                <i
                  ...
                  :class="{
                    'baku-button primary-button': isPlaying !== 'selection',
                    'disabled-button': isPlaying === 'selection',
                  }"
                  @click="playAnimation()"
                ...
                <i
                  class="
                    toolbar-button toolbar-button-big
                    icon-pause
                    baku-button
                  "
...
export default class CaptureView extends AbstractProjectView {
  ...
  get IsFrameLiveView() {
    return (
      !this.isPlaying && this.activeFrame === this.getActiveShot?.images.length
    );
  }

  private onImagePreloaded(imageId: string): void {
    if (this.getActiveShot.images.length !== this.activeFrame && this.getActiveShot.images[this.activeFrame].id === imageId) {
      this.displayFrame(this.activeFrame);
    }
