<template>
  ...
  <div v-else>{{ socketStatus === 'error' ? 'Erreur de connexion au serveur' : 'Synchronisation non supportée par votre navigateur'}}</div>
</template>
...
export default class SmartphoneView extends Vue {
  ...
  public mounted() {
    ...
    (!!(navigator as any).mozGetUserMedia && !!(navigator as any).mozRTCPeerConnection) ||
