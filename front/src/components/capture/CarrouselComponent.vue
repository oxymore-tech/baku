  ...
  <div class="carrousel-root-div">
    <div class="toolbar">
      ...
      <div
        ...
        :class="{
          disabled:
            isFrameLiveView || !imagesToCopy.length || isPlaying || !canEdit,
        }"
      >
      ...
      <div
        ...
        :class="{
          disabled: isFrameLiveView || !isMultiSelect || isPlaying || !canEdit,
        }"
      >
    ...
    <div ref="carrouselContainer" class="carrousel-container">
      <template v-for="(image, index) in computedPreviousShotImages">
        ...
        <template v-else>
          <div
            :key="'left' + index + '-prev'"
            class="image-container image-container-empty"
          >
            <div class="carrousel-thumb" style="width: 100%; border: none" />
          </div>
        </template>
      </template>
      ...
      <template v-for="(image, index) in computedLeftCarrousel">
        <template v-if="image !== null">
          <div :key="'left' + index">
            <span class="framenumber-indicator"
              >{{
                "" + (activeImage + 1 - (computedLeftCarrousel.length - index))
              }}
            </span>
            <div
              class="image-container"
              :class="{
                activeSelection: selectedImagesForReal.includes(
                  activeImage - (computedLeftCarrousel.length - index)
                ),
              }"
            >
              <img
                ...
                :src="ImageCacheService.getThumbnail(image.id)"
                @click="
                  moveToImage($event, index - computedLeftCarrousel.length)
                "
              />
            </div>
          </div>
      ...
      <template v-if="computedActiveImage">
        <div id="carrouselActiveImg" ref="carrouselActiveImg">
          <span class="framenumber-indicator-active">{{
            "" + (activeImage + 1)
          }}</span>
      ...
      <div
        id="carrouselActiveImg"
        class="image-container active active-capure"
        style="position: relative"
        v-else
      >
        <div class="carrousel-thumb active" :v-if="canEdit">
