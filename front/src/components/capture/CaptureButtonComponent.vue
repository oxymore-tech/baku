<template>
  <div style="display:flex;">
    <audio ref="audio" src="@/assets/camera_shutter.mp3" preload="auto"/>
    <i class="icon-camera" v-if="canCapture && !protectCapture" @click="capture()"
       style="color:#e66359; font-size: 3rem"/>
    <i class="icon-camera" style="cursor:default" v-else-if="canCapture && protectCapture"/>
    <img v-else src="@/assets/camera-off-color.svg" style="height:32px; max-width:unset"
         @click="moveToCapture()"/>
  </div>
</template>
...
<script lang="ts">
  ...
  export default class CaptureButtonComponent extends Vue {
    ...
    public async mounted() {
      ...
      request.open('GET', '/assets/camera_shutter.mp3', true);
    ...
    public capture() {
      ...
      console.log("after", +new Date() - start);
    }
