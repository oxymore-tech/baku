...
export default class WebcamCaptureComponent extends Vue {
  ...
  private capture() {
    ...
    const video = document.getElementById("remoteVideo") as any;
    canvas.width = 640;
    canvas.height = 480;
    ...
    request.open(
      "POST",
      `https://${location.hostname}/back/${this.$store.state.plan.activePlan}/upload/`
    );
    const self = this;
    request.onreadystatechange = function() {
      ...
      if (this.readyState === XMLHttpRequest.DONE && this.status === 200) {
        ...
        self.$store.commit("plan/addNewPicture", pictureId);
      }
    };
